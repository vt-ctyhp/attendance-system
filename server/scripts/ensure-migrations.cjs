...
const runPrisma = (command, databaseUrl) => {
...
const markBaselineMigrationsApplied = async (databaseUrl, baselineMigrations) => {
  ...
  for (const migration of baselineMigrations) {
    runPrisma(`npx prisma migrate resolve --applied ${migration}`, databaseUrl);
  }
};
...
const verifyColumns = async (databaseUrl) => {
  if (dryRun) {
  ...
  await withClient(databaseUrl, async (client) => {
    for (const { table, column } of verificationTargets) {
  ...
  info('Verified UTO columns are present');
};
...
const ensureMigrations = async () => {
  ...
  info(`Ensuring migrations for ${redactUrl(databaseUrl)}`);
  ...
  if (!hasMigrationsTable) {
    if (!hasUserTable) {
      info('No existing Prisma metadata detected; running db push baseline');
      runPrisma('npx prisma db push', databaseUrl);
      await markBaselineMigrationsApplied(databaseUrl, migrations);
      await verifyColumns(databaseUrl);
      return;
    }
  ...
  runPrisma('npx prisma migrate deploy', databaseUrl);
  await verifyColumns(databaseUrl);
};
