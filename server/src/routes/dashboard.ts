...
import {
  listBonusesForPayDate,
  recalcMonthlyBonuses,
  updateKpiBonusStatus
} from '../services/payroll/bonuses';
import {
...
const formatCsvDateTime = (value: Date) => formatInTimeZone(value, DASHBOARD_TIME_ZONE, CSV_DATE_TIME);
const formatCsvDate = (value: Date) => formatInTimeZone(value, DASHBOARD_TIME_ZONE, CSV_DATE);
const formatCsvDateList = (values: Date[]) =>
  values.length ? values.map((value) => formatCsvDateTime(value)).join('; ') : '';
...
const payrollStatusClasses: Record<PayrollStatus, string> = {
...
const normalizePayrollStatus = (status: string): PayrollStatus =>
  (PAYROLL_STATUSES as readonly string[]).includes(status as PayrollStatus)
    ? (status as PayrollStatus)
    : 'draft';

const bonusTypeLabels: Record<string, string> = {
  [BONUS_TYPE_MONTHLY]: 'Monthly Attendance',
  [BONUS_TYPE_QUARTERLY]: 'Quarterly Attendance',
  [BONUS_TYPE_KPI]: 'KPI Bonus'
};

const weekdayLabels = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

const formatDayRange = (start: number, end: number) =>
  start === end ? weekdayLabels[start] : `${weekdayLabels[start]}–${weekdayLabels[end]}`;
...
const summarizeSchedule = (schedule: unknown) => {
  const normalized = ensureSchedule(schedule);
  const groups = new Map<
    string,
    { start: string; end: string; hours: number; breakMinutes: number; days: number[] }
  >();
...
const computeNextPayrollPayDate = () => {
  ...
  const endOfMonthDate = endOfMonth(nowZoned);
  ...
  return formatIsoDate(zonedTimeToUtc(nextZoned, DASHBOARD_TIME_ZONE));
};

const toPayDateParam = (value: Date) => formatIsoDate(zonedTimeToUtc(value, DASHBOARD_TIME_ZONE));
...
const buildRosterRow = (
  ...
  dayEnd: Date,
  now: Date,
  scheduleInfo: { start: string; enabled: boolean } | null
): TodayRosterRow => {
...
const fetchTodayRosterData = async (referenceDate: Date, sessions: SessionRecord[]): Promise<TodayRosterData> => {
  ...
  const scheduleByUser = new Map<number, { start: string; enabled: boolean } | null>();
  ...
  if (userIds.length) {
    ...
    const weekday = utcToZonedTime(dayStart, PAYROLL_TIME_ZONE).getDay();
  ...
  const rows = users.map((user) =>
    buildRosterRow(
      ...
      dayEnd,
      now,
      scheduleByUser.get(user.id) ?? null
    )
  );
...
const computeRosterTardyMinutes = (scheduledStart: string, actualStart: Date) => {
  ...
  if (differenceInCalendarDays(zoned, scheduled) !== 0) {
    return 0;
  }
...
const baseStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .live-roster-card__header { position: sticky; top: 0; display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 16px 24px; background: #F9FAFB; border-bottom: 1px solid var(--border); z-index: 1; }
  ...
  .live-roster-refresh { border: 1px solid var(--border); background: white; color: var(--text-primary); border-radius: 6px; width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center; font-size: 16px; cursor: pointer; transition: background 0.15s; }
  .live-roster-refresh:hover:not(:disabled) { background: #F9FAFB; }
  ...
  .live-roster-table thead th { position: sticky; top: 0; background: #F9FAFB; color: var(--text-secondary); font-size: 12px; font-weight: 600; padding: 12px 16px; border-bottom: 1px solid var(--border); white-space: nowrap; }
  ...
  .live-roster-table tbody tr:hover td { background: #F9FAFB; }
  ...
  .roster-avatar { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 6px; background: #EFF6FF; color: var(--primary); font-weight: 600; font-size: 13px; text-transform: uppercase; }
  ...
  .live-roster-card__body .empty { margin: 24px; padding: 48px 24px; border: 1px dashed var(--border); background: #F9FAFB; color: var(--text-muted); font-size: 13px; border-radius: 6px; }
...
dashboardRouter.get(
  '/shifts',
  asyncHandler(async (req, res) => {
    const parseLookahead = (value: unknown) => {
    ...
    type ScheduleKind = 'shift' | 'pto' | 'uto' | 'make_up';

    const TYPE_LABELS: Record<ScheduleKind, string> = {
    ...
    const mapRequestKind = (type: string): ScheduleKind => {
      switch (type) {
        case 'pto':
          return 'pto';
        case 'uto':
    ...
    const totals: Record<ScheduleKind, number> = {
      shift: 0,
      pto: 0,
      uto: 0,
      make_up: 0
...
dashboardRouter.get('/balances', async (req, res) => {
  ...
  if (selectedUser) {
    ...
    const userRule = await prisma.accrualRule.findUnique({ where: { userId: selectedUser.id } });
    const defaultRule = await prisma.accrualRule.findFirst({ where: { isDefault: true } });
    const resolveAccrual = (selector: (rule: typeof userRule) => number | undefined | null) => {
      if (userRule) {
    ...
    const summaryCards = `
      <div class="summary-cards">
        <div class="summary-card">
          ...
          <div class="summary-meta">Baseline ${escapeHtml(formatHours(basePto))} h${
            ptoAccrual > 0 ? ` • Accrues ${escapeHtml(formatHours(ptoAccrual))} h/mo` : ''
          }</div>
        </div>
        <div class="summary-card">
          <div class="summary-title">UTO Remaining</div>
          <div class="summary-value">${escapeHtml(formatHours(currentUto))} h</div>
          <div class="summary-meta">Baseline ${escapeHtml(formatHours(baseUto))} h${
            utoAccrual > 0 ? ` • Accrues ${escapeHtml(formatHours(utoAccrual))} h/mo` : ''
          }</div>
        </div>
        <div class="summary-card">
...
dashboardRouter.get('/payroll', async (req, res) => {
  ...
  const latestConfigByUser = new Map<number, EmployeeCompSnapshot>();
  ...
  const buildStatusMeta = (period: (typeof periods)[number]) => {
    const parts: string[] = [];
    if (period.approvedBy && period.approvedAt) {
      parts.push(
        `Approved by ${escapeHtml(period.approvedBy.name)} on ${escapeHtml(formatDateTime(period.approvedAt))}`
      );
    }
  ...
  const periodRows = periods
    .map((period) => {
      ...
      const totalsEntries: Array<{ key: keyof PeriodTotals; label: string }> = [
        { key: 'base', label: 'Base' },
      ...
      const totalsGrid = totalsEntries
        .map(({ key, label }) =>
          `<div><dt>${escapeHtml(label)}</dt><dd>${formatCurrency(totals[key])}</dd></div>`
        )
        .join('');
      ...
      const computedMeta = period.computedAt
        ...
        : '';
      const statusChip = `<span class="${payrollStatusClasses[status]}">${escapeHtml(
  ...
  const makeNumberValue = (value: number | undefined) =>
  ...
  const bonusTypeGroup = (type: string) => bonusTypeLabels[type] ?? type;
  const kpiCandidates = bonusCandidates.filter((candidate) => candidate.type === BONUS_TYPE_KPI);
  const otherBonuses = bonusCandidates.filter((candidate) => candidate.type !== BONUS_TYPE_KPI);
        ...
        <script>
          (() => {
            ...
            const serializeSchedule = (form) => {
              ...
              const timeZone = form.getAttribute('data-default-timezone') || '${PAYROLL_TIME_ZONE}';
...
const updateEmployeeNameSchema = z.object({
  name: z.string().min(1).max(200)
});
...
dashboardRouter.get('/payroll/employees', async (req, res) => {
  const employees = await prisma.user.findMany({
    orderBy: { name: 'asc' },
    include: {
      balance: {
...
dashboardRouter.get('/employees/:employeeId', async (req, res) => {
  ...
  const configs = await listEmployeeConfigs(employee.id);
  const latestConfig = configs[0] ?? null;
  const scheduleSnapshot = latestConfig ? ensureSchedule(latestConfig.schedule) : ensureSchedule({});
  const defaultEffectiveDate = formatIsoDate(zonedStartOfDay(new Date()));

  const toNumberInput = (value: number | null | undefined) =>
