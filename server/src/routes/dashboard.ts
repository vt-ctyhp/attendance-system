...
const summarizeSchedule = (schedule: Record<string, WeekdaySchedule>) => {
  const normalized = ensureSchedule(schedule);
  const groups = new Map<
    string,
    { start: string; end: string; hours: number; days: number[] }
  >();
...
dashboardRouter.get('/payroll', async (req, res) => {
  ...
  const scheduleTemplate = (() => {
    ...
    const templateReference = enabledDays.includes('1')
      ? baseSchedule['1']
      : enabledDays.length
        ? baseSchedule[enabledDays[0] as keyof typeof baseSchedule]
        : baseSchedule['1'];
    return {
    ...
    ? latestConfigs
        .map((config) => {
          ...
          const editQuery = new URLSearchParams({
            factsMonth: selectedFactsMonth,
            bonusDate: bonusDateParam,
            employeeId: String(config.userId)
          });
          const editHref = `/dashboard/payroll?${editQuery.toString()}#update-compensation`;
          return `
            <tr>
              <td>
              ...
              <td class="actions-cell"><a class="button button-secondary" href="${escapeHtml(editHref)}">Edit</a></td>
            </tr>
          `;
        })
        .join('\n')
    : '<tr><td colspan="9" class="empty">No compensation configurations have been recorded.</td></tr>';
